 ...
 * @since 2.0.0
 */
export const reduce: Foldable1<URI>['reduce'] = (b, f) => (fa) => {
  let r = f(b, fa.value)
  const len = fa.forest.length
  for (let i = 0; i < len; i++) {
    r = reduce(r, f)(fa.forest[i])
  }
  return r
}
 ...
 * @since 2.0.0
 */
export const foldMap: Foldable1<URI>['foldMap'] = (M) => (f) => reduce(M.empty, (acc, a) => M.concat(acc, f(a)))
